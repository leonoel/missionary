...
(defn violated
  ([nm msg]
   (throw #?(:clj  (ProtocolViolation. (str (pr-str nm) " flow protocol violation: " msg))
             :cljs (ex-info (str (pr-str nm) " flow protocol violation: " msg) {}))))
  ([nm msg e]
   (throw #?(:clj  (ProtocolViolation. (str (pr-str nm) " flow protocol violation: " msg) e)
             :cljs (ex-info (str (pr-str nm) " flow protocol violation: " msg) {} e)))))

(defn- cannot-throw [violated nm f-name f]
  (try (f) (catch #?(:clj Throwable :cljs :default) e
             (violated nm (str f-name " cannot throw") e))))

(defn flow
  ([input-flow] (flow violated "" input-flow))
  ([nm input-flow] (flow violated nm input-flow))
  ([violated nm input-flow]
   ...
   (fn [step done]
     (let [!should-step? (atom ::init), !done? (atom false), !crashed? (atom false), !v (atom ::init)
           step (fn []
                  ...
                  (if (first (reset-vals! !done? true)) (violated nm "done called twice") (cannot-throw violated nm "done" done)))
